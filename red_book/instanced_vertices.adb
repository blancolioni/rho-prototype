...
with Rho.Matrices;
with Rho.Mouse;
with Rho.Node.Instanced;
with Rho.Scene;
with Rho.Shapes;
with Rho.Render_Window;

package body Instanced_Vertices is

   type Frame_Listener is
     new Rho.Frame_Event.Rho_Frame_Listener_Interface with
      record
         Top_Node     : Rho.Node.Instanced.Rho_Instanced_Node;
   ...
   procedure Create_Window is
      ...
      Top_Node    : constant Rho.Node.Instanced.Rho_Instanced_Node :=
                      Rho.Node.Instanced.Create ("top node");
   ...
   begin
      ...
      for I in Instance_Range loop
         Top_Node.Append
           (Position => Node_Offset (I));
      end loop;

      Scene.Append_Node (Top_Node);
