...
void Rho_uniform_matrix_f
  (GLint location, GLsizei size, GLboolean transpose,
...
{
  GLfloat mat[] = {a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p};
  glUniformMatrix4fv(location,size,transpose,mat);
}

void Rho_glVertexAttribDivisor(GLuint index, GLuint divisor)
{
  glVertexAttribDivisor(index, divisor);
}

void Rho_glDrawArraysInstanced
  (GLenum mode,
  GLint first,
  GLsizei count,
  GLsizei primcount)
...
void Rho_shader_source(GLuint shader, const GLchar *source)
{
  glShaderSource(shader, 1, &source, NULL);
}

GLint
Rho_get_compile_status (GLuint shader)
{
  GLint status;
